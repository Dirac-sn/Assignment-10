...
#dirichlet bc
solve_cnt(0,1,1/3,1/4,'d',10**(-8),'Dirichlet BC')
plot_s(0, 1,1/3,1/4,'d',10**(-8),'Dirichlet BC')
#neumann Bc
solve_cnt(0,1,-1/9,-1/16,'n',10**(-3),'Neumann BC')

#Robin 1 
solve_cnt(0,1,-2/9,1/4,'r',10**(-3),'Robin-1 BC',1/3)  
plot_s(0, 1,-2/9,1/4,'r',10**(-6),'Robin-1 BC',1/3)
#Robin 2
solve_cnt(1,0,3/16,1/3,'r',10**(-3),'Robin-2 BC',-1)
plot_s(1,0,3/16,1/3,'r',10**(-3),'Robin-2 BC',-1)


def Error(a,b,f,N,tol):
    mat = linearshooting(a,b,1/3,1/4,N,slope,0,1,tol)[0]
    x = mat[0]
    h = x[1] - x[0]
    ynum = mat[1][:,0]
    E_l = abs(ynum - f(x)) 
...
data2 = {'err':['E_max','E_rms'],'h':Yu,'N':tu}

df3 = pd.DataFrame(data2)
df3.to_csv('slope.csv')
print(df3)
